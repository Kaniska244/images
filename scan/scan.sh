...
check_prerequisites() {
    ...
    if ! docker scout version &>/dev/null; then
        ...
        if ! docker scout version &>/dev/null; then
            echo -e "${RED}Error: Failed to install Docker Scout. Docker Scout is required.${NC}"
            exit 1
        fi
    fi
...
# Generate SBOM for an image using Docker Scout
generate_sbom() {
    local image="$1"
    local output_file="$2"
    ...
    # Use Docker Scout for SBOM generation
    if docker scout version &>/dev/null; then
        ...
        if timeout "$SCOUT_TIMEOUT" docker scout sbom "$scout_image" \
            ...
            if [ -f "$temp_output" ] && [ -s "$temp_output" ]; then
                if jq -e '.source' "$temp_output" > /dev/null 2>&1; then
...
# Get CVEs for an image with detailed info using Docker Scout
get_cves_with_details() {
    ...
    # Use Docker Scout for CVE scanning
    if docker scout version &>/dev/null; then
        ...
        if timeout "$SCOUT_TIMEOUT" docker scout cves "$scout_image" \
            ...
            if [ -f "$temp_output" ] && [ -s "$temp_output" ]; then
                if jq -e '.runs[0].results' "$temp_output" > /dev/null 2>&1; then
                    cp "$temp_output" "$output_file"
...
filter_vulnerabilities() {
    ...
    # Parse vulnerabilities (Docker Scout SARIF format)
